...
export default function Hero({ onNavigate }: HeroProps) {
  ...
  return (
    <div className="relative bg-[#F7F5F0] text-[#141416] min-h-[calc(100vh-140px)] flex flex-col justify-between" id="gec-landing-page">
      
      {/* Subtle Warm Stone Background Ambience */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#EFEAE1]/70 via-[#F7F5F0] to-[#F7F5F0] pointer-events-none" />
      <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[800px] h-[450px] bg-[#A36B3B]/5 rounded-full blur-[150px] pointer-events-none" />

      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-8 pb-20">
        
        {/* Expanded Grand Architectural Hero Frame with Stage Choir Image */}
        <div className="relative rounded-3xl overflow-hidden border border-[#E4DCD0] shadow-xl shadow-stone-900/10 mb-14 min-h-[560px] sm:min-h-[640px] lg:min-h-[720px] flex items-center justify-center">
          
          {/* Dynamic Motion Background Image Layer with Ken Burns Effect */}
          ...
          {/* Heading Content Overlay - Pure White Heading & Welcome Home */}
          <div className="relative z-10 text-center max-w-4xl mx-auto space-y-6 px-6 py-20 sm:py-28 lg:py-36">
                ...
                <div className="flex items-center justify-center select-none">
                  ...
                  <div className="inline-flex items-center">
                    {['W', 'E', 'L', 'C', 'O', 'M', 'E'].map((char, idx) => {
                      const distFromCenter = 6 - idx + 1; // 1 (closest 'E') to 7 (outermost 'W')
                  ...
                  <div className="inline-flex items-center">
                    {['H', 'O', 'M', 'E'].map((char, idx) => {
                      const distFromCenter = idx + 1; // 1 (closest 'H') to 4 (outermost 'E')
        ...
        {/* Featured Upcoming Spotlight Card (Alabaster Card & Bronze Accent) */}
        <motion.div
          initial={{ opacity: 0, y: 25 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.35 }}
          className="max-w-4xl mx-auto rounded-3xl bg-white border border-[#E4DCD0] p-6 sm:p-8 shadow-xl shadow-stone-900/5 relative overflow-hidden"
        >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
            <div className="space-y-2 text-center md:text-left">
              ...
              <p className="text-xs sm:text-sm text-[#54575E]">
                October 1st – 4th, 2026 • In-Person & Global Broadcast
              </p>
            </div>
        ...
        {/* 3 Main Direct Gateways: Who are we, Get edified here, Worship with us */}
            ...
            {corePillars.map((pillar, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative overflow-hidden border rounded-3xl p-6 sm:p-7 cursor-pointer transition-all duration-300 group flex flex-col justify-between ${pillar.cardBg} ${pillar.cardBorder} ${pillar.cardShadow}`}
                ...
                >
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-semibold tracking-wider uppercase border ${pillar.badgeColor}`}>
                        {pillar.badge}
                      </span>
                    </div>

                    <h4 className={`font-cinzel font-bold text-xl mb-2.5 transition-colors ${pillar.titleColor}`}>
                      {pillar.title}
                    </h4>
                  ...
                  <div className={`relative z-10 mt-6 pt-4 border-t ${pillar.borderTop} flex items-center justify-between text-xs font-bold uppercase font-display tracking-wider transition-colors ${pillar.ctaColor}`}>
